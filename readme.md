PHP User
--------

PHP User library.
[Github-Repo](https://github.com/benniweber/PHPUser "PHPUser Repo").
Warning: This library is not finished, and still under development!

LICENSE
--------

The PHPUser library itself is licensed under the permissive MIT-License, see file MIT-License for more information, 
or visit [this site](http://www.opensource.org/licenses/MIT "More information about the MIT-LICENSE").

PHPUser uses the phpass library, which was put in the public domain. You can find more information about this useful 
password-hasher [here](http://www.openwall.com/phpass/ "Portable PHP password hashing framework"). 
For your convenience, this library is includes in PHPuser's github-repo 